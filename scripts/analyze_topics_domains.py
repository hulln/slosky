...
Public-safe outputs (default: outputs/topic_analysis/):
  - domain_category_summary.csv
  - figure_domain_categories.png

Detailed hashtag/domain tables, figures and prose require --private-output-dir.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--private-output-dir", type=Path,
        help="Opt in to detailed hashtag/domain tables, figures, and prose.",
    )
...
def write_summary(
    ...
    lines += [
        ...
        "## Domain overview",
        "",
        f"Posts with at least one extracted external link domain: {posts_with_link:,} "
...
def _bullets(
    ...
    bullets = [
        ...
        f"({posts_with_link/total_posts*100:.1f}\\%) have at least one extracted external link domain.",
        ...
        f"Extracted external link-domain rates differ by post type: "
        f"{orig_share:.1f}\\% for original posts compared with {reply_share:.1f}\\% for replies, "
...
def _paragraph(
    ...
    return (
        ...
        f"vs.\\ {reply_share_val:.1f}\\% of replies have an extracted external link domain), consistent "
...
def main() -> int:
    ...
    if args.private_output_dir:
        ...
        hashtag_df.to_csv(args.private_output_dir / "hashtag_summary.csv", index=False, float_format="%.4f")
        domain_df.to_csv(args.private_output_dir / "domain_summary.csv", index=False, float_format="%.4f")
        figure_top_hashtags(hashtag_df, args.private_output_dir)
        figure_top_domains(domain_df, args.private_output_dir)
        summary_path = write_summary(
            args.private_output_dir, total, hashtag_df, posts_with_hashtag,
    ...
    print(f"Posts with extracted external link domain: {posts_with_link:,} ({posts_with_link/total*100:.1f}%)")
    if args.private_output_dir:
