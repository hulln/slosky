...
def main() -> int:
    ...
    parser.add_argument(
        "--private-output-dir",
        type=Path,
        help="Opt in to author-level and detailed hashtag/domain CSV files.",
    )
    ...
    with growth_path.open("w", encoding="utf-8", newline="") as fh:
        ...
        for month in sorted(monthly_posts):
            ...
            authors = len(monthly_active_authors[month])
            w.writerow([month, posts if posts >= 5 else "<5", authors if authors >= 5 else "<5"])
    print(f"  → {growth_path}")
    ...
    if args.private_output_dir:
        ...
        private_growth_path = args.private_output_dir / "paper_growth.csv"
        with private_growth_path.open("w", encoding="utf-8", newline="") as fh:
            w = csv.writer(fh)
            w.writerow(["month", "posts", "unique_authors"])
            for month in sorted(monthly_posts):
                w.writerow([month, monthly_posts[month], len(monthly_active_authors[month])])

        first_seen_path = args.private_output_dir / "paper_author_first_seen.csv"
    ...
    if args.private_output_dir:
        author_path = args.private_output_dir / "paper_authors.csv"
        with author_path.open("w", encoding="utf-8", newline="") as fh:
            w = csv.writer(fh)
            w.writerow(
                [
                    "rank", "handle", "posts", "reply_rate", "months_active",
                    "first_post", "last_post", "handle_type", "custom_domain", "pds",
                ]
            )
            for rank, row in enumerate(author_rows, 1):
                w.writerow(
                    [rank, row["handle"], row["posts"], row["reply_rate"],
                     row["months_active"], row["first_post"], row["last_post"],
                     row["handle_type"], row["custom_domain"], row["pds"]]
                )

    handle_types = Counter(row["handle_type"] for row in author_rows)
    pds_types = Counter()
    for row in author_rows:
        pds_host = normalize_host(row["pds"])
        if pds_host.endswith(".host.bsky.network") or pds_host == "host.bsky.network":
            pds_types["bluesky_hosting"] += 1
        elif pds_host == "eurosky.social":
            pds_types["eurosky_social"] += 1
        else:
            pds_types["other_or_unknown"] += 1
    custom_posts = sum(row["posts"] for row in author_rows if row["handle_type"] == "custom_domain")
    custom_eurosky = sum(
        1 for row in author_rows
        if row["handle_type"] == "custom_domain" and normalize_host(row["pds"]) == "eurosky.social"
    )
    infrastructure = {
        "authors": n_authors,
        "pds_types": dict(pds_types),
        "handle_types": dict(handle_types),
        "custom_domain_posts": custom_posts,
        "custom_domain_posts_pct": pct(custom_posts, total),
        "custom_handle_and_eurosky_hosting": custom_eurosky,
        "input_stage": "final corpus plus private PDS-resolution cache",
        "generating_script": "scripts/analyze_for_paper.py",
    }
    infrastructure_path = args.output_dir / "infrastructure_summary.json"
    infrastructure_path.write_text(json.dumps(infrastructure, indent=2) + "\n", encoding="utf-8")

    # ── 5. topics: hashtags + domains ─────────────────────────────────────────
    if args.private_output_dir:
        hashtag_path = args.private_output_dir / "paper_hashtags.csv"
        ...
        domain_path = args.private_output_dir / "paper_domains.csv"
