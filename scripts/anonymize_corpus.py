...
Usage:
    ...
    python scripts/anonymize_corpus.py \
        ...
        --mapping-json outputs/private/author_mapping.json
"""
...
AT_URI_RE = re.compile(r"(?i)\bat://[^\s<>\"']+")
DID_RE = re.compile(r"(?i)\bdid:(?:plc|web|key):[A-Za-z0-9._:%-]+")
BLUESKY_HANDLE_RE = re.compile(
    r"(?i)(?<![\w@])@?[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?"
    r"\.(?:bsky\.social|eurosky\.social)(?![\w.-])"
)
...
def redact_text_identifiers(text: str) -> str:
    """Remove identifier forms not covered by ordinary @-mention replacement."""
    text = AT_URI_RE.sub("[at-uri]", text)
    text = DID_RE.sub("[did]", text)
    return BLUESKY_HANDLE_RE.sub("[handle]", text)


def assert_no_text_identifiers(text: str, post_id: str) -> None:
    """Abort rather than write an output row containing a known identifier form."""
    for label, pattern in (
        ("AT URI", AT_URI_RE),
        ("DID", DID_RE),
        ("Bluesky handle", BLUESKY_HANDLE_RE),
    ):
        if pattern.search(text):
            raise ValueError(f"Unredacted {label} in {post_id}; output is not safe")


def scan_generated_jsonl(path: Path) -> None:
    """Fail after generation if a known identifier form remains in output text."""
    with path.open(encoding="utf-8") as handle:
        for line_number, line in enumerate(handle, start=1):
            row = json.loads(line)
            assert_no_text_identifiers(
                str(row.get("text") or ""), str(row.get("post_id") or line_number)
            )
...
def main() -> int:
    ...
    try:
        with args.input.open(encoding="utf-8") as in_f, \
            ...
            for line in in_f:
                ...
                anon_text = redact_text_identifiers(anon_text)
                assert_no_text_identifiers(anon_text, post_id)
    ...
    scan_generated_jsonl(args.output_jsonl)
