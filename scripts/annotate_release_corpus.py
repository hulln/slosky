...
def classla_only_row(row: dict[str, Any], sentences: list[dict[str, Any]]) -> dict[str, Any]:
    """Fall back to a CLASSLA-only annotation when Trankit cannot be reconciled.

    Used when CLASSLA and Trankit disagree on sentence or token boundaries for a
    single post. The post is still annotated, but UPOS/FEATS come from CLASSLA and
    the dependency layer is left empty, so the provenance recorded in the output
    stays honest about which tool produced which column.
    """
    ...
    for sentence in sentences:
        ...
        merged_sentences.append({"id": sentence["id"], "text": sentence["text"], "tokens": tokens})
...
def annotate_row(row: dict[str, Any], classla_pipeline: Any, trankit_pipeline: Any) -> dict[str, Any]:
    """Annotate one post with the hybrid pipeline.

    If Trankit cannot be aligned to the CLASSLA tokenization for this post, the post
    is annotated with CLASSLA alone rather than aborting the run. Over a 141k-post
    batch a handful of pathological posts is expected; losing the whole job to one of
    them is not acceptable, and silently dropping them would bias the corpus.
    """
    text = row.get("text") or ""
    classla_doc = classla_pipeline(text)
    sentences = classla_sentences(classla_doc)
    if trankit_pipeline is None:
        return classla_only_row(row, sentences)
    try:
        return merge_with_trankit(row, sentences, trankit_pipeline)
    except ValueError as exc:
        fallback = classla_only_row(row, sentences)
        fallback["annotation"]["trankit_fallback"] = True
        fallback["annotation"]["trankit_fallback_reason"] = str(exc)
        return fallback


def merge_with_trankit(
    row: dict[str, Any], sentences: list[dict[str, Any]], trankit_pipeline: Any
) -> dict[str, Any]:
...
def write_conllu_sentence(handle: Any, row: dict[str, Any], sentence: dict[str, Any]) -> None:
    ...
    # Provenance must follow what actually ran for this post, not what the default
    # pipeline would have done: a --skip-trankit run or a per-post fallback produces
    # no UD layer at all, and a released CoNLL-U claiming one would misstate itself.
    annotation = row.get("annotation", {})
    if annotation.get("upos") == "trankit":
        handle.write("# ud = Trankit on CLASSLA-pretokenized input\n")
    else:
        handle.write("# ud = none (CLASSLA XPOS/MSD only)\n")
...
def write_meta(args: argparse.Namespace, counts: dict[str, int], modules: dict[str, Any]) -> None:
    meta = {
        ...
        "resumed": args.resume,
        "counts": counts,
        "fallback_json": str(args.fallback_json),
        "pipeline": {
            "sentence_segmentation": "CLASSLA",
            "tokenization": "CLASSLA",
            "lemma": "CLASSLA",
            "xpos": "CLASSLA",
            "upos": "CLASSLA" if args.skip_trankit else "Trankit",
            "feats": "CLASSLA" if args.skip_trankit else "Trankit",
            "dependencies": None if args.skip_trankit else "Trankit",
            "trankit_input": None if args.skip_trankit else "CLASSLA-pretokenized sentences",
        },
        "settings": {
            "classla_language": "sl",
...
def completed_post_ids(path: Path) -> set[str]:
    """Post ids already fully written, repairing a partial tail in the process.

    A run killed mid-write leaves a truncated final line. Appending after it would
    splice the next post onto the fragment, producing one corrupt record and losing
    another -- so the file is rewritten without the fragment before we append.
    """
    done: set[str] = set()
    if not path.exists():
        return done
    ...
    with path.open(encoding="utf-8") as handle:
        for line in handle:
            ...
            try:
                ...
                truncated = True
                continue
            key = row.get("post_id") or row.get("uri")
            if key:
                done.add(str(key))
    ...
    if truncated:
        print(
            f"  repairing {path.name}: dropped an incomplete trailing record",
            file=sys.stderr,
        )
...
def resync_conllu(path: Path, keep: set[str]) -> None:
    """Rewrite a CoNLL-U file to exactly the posts present in the JSONL.

    The two streams are written independently, so an interrupted run can leave the
    CoNLL-U ahead of or behind the JSONL. Since the JSONL decides what gets skipped
    on resume, any drift here would silently duplicate or drop sentences. Rebuilding
    from the `# post_id` comments keeps the two in lockstep.
    """
    if not path.exists() or not keep:
        return
    blocks_kept = 0
    tmp = path.with_suffix(path.suffix + ".resync")
    with path.open(encoding="utf-8") as src, tmp.open("w", encoding="utf-8") as dst:
        ...
        if block and post_id in keep:
            ...
            blocks_kept += 1
    tmp.replace(path)
    print(f"  resynced {path.name}: kept {blocks_kept:,} sentence blocks", file=sys.stderr)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--fallback-json",
        type=Path,
        default=DEFAULT_OUTPUT_JSONL.with_suffix(DEFAULT_OUTPUT_JSONL.suffix + ".fallbacks.json"),
        help="Where to record posts that fell back to CLASSLA-only annotation.",
    )
    parser.add_argument(
        "--classla-type",
    ...
    parser.add_argument(
        "--skip-trankit",
        action="store_true",
        help="Annotate with CLASSLA only (tokenize, pos, lemma), skipping the Trankit "
        "UD layer entirely. Much faster, and sufficient for the corpus statistics "
        "reported in the paper, which are all derived from the CLASSLA XPOS/MSD column.",
    )
    parser.add_argument(
        "--resume",
        action="store_true",
        help="Append to existing output files, skipping posts already annotated. "
        "Use this to continue a run that was interrupted.",
    )
    parser.add_argument(
...
def main() -> int:
    ...
    args.fallback_json.parent.mkdir(parents=True, exist_ok=True)

    already_done: set[str] = set()
    if args.resume:
        already_done = completed_post_ids(args.output_jsonl)
        resync_conllu(args.output_conllu, already_done)
        print(f"Resuming: {len(already_done):,} posts already annotated.", file=sys.stderr)

    if args.skip_trankit:
        trankit_module, trankit_pipeline = None, None
        print("Skipping Trankit: CLASSLA-only annotation.", file=sys.stderr)
    else:
        trankit_module, trankit_pipeline = init_trankit_pipeline(args)
    classla_module, classla_pipeline = init_classla_pipeline(args)

    counts = {"posts": 0, "sentences": 0, "tokens": 0, "trankit_fallbacks": 0}
    fallbacks: list[dict[str, str]] = []
    mode = "a" if args.resume else "w"
    with args.output_jsonl.open(mode, encoding="utf-8") as jsonl_handle, args.output_conllu.open(
        mode, encoding="utf-8"
    ) as conllu_handle:
        for row in iter_jsonl(args.input):
            if args.limit is not None and counts["posts"] >= args.limit:
                break
            post_key = str(row.get("post_id") or row.get("uri") or "")
            if post_key and post_key in already_done:
                continue
            ...
            if annotated["annotation"].get("trankit_fallback"):
                counts["trankit_fallbacks"] += 1
                fallbacks.append(
                    {
                        "post_id": post_key,
                        "reason": annotated["annotation"].get("trankit_fallback_reason", ""),
                    }
                )
            if args.progress_every and counts["posts"] % args.progress_every == 0:
                # Flush both handles so an interrupted run loses at most one block
                # and --resume can pick up from a clean boundary.
                jsonl_handle.flush()
                conllu_handle.flush()
                print(
                    f"Annotated {counts['posts']:,} posts, "
                    f"{counts['sentences']:,} sentences, {counts['tokens']:,} tokens, "
                    f"{counts['trankit_fallbacks']:,} Trankit fallbacks",
                    file=sys.stderr,
                )

    args.fallback_json.write_text(
